...
const snippets = [];
mongoose.connect(
  `mongodb+srv://paranoia:${process.env.DB_PASS}@userinexperience-maztg.mongodb.net/test`
);
...
app.get("/app/snippets", async (req, res) => {
  const [err, snippets] = await to(Snippet.find({}).exec());
  if (err) return res.status(400).json({ error: err });
  return res.render("snippets.ejs", { snippets });
});
